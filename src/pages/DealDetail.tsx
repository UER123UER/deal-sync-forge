...
import { ChevronDown, ChevronRight, Edit, Eye, Mail, Plus, FileText, MoreVertical, GripVertical, Download, Printer, Send, Trash2, MessageSquare, Bell } from 'lucide-react';
...
export default function DealDetail() {
  ...
  const sellerAgent = deal.contacts.find((c) => c.role.includes('Agent'));
  const seller = deal.contacts.find((c) => c.role === 'Seller' || c.role === 'Buyer');
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                <button className="text-sm text-primary hover:underline flex items-center gap-1">
                  <Plus className="w-3 h-3" /> Add a New Contact
                </button>
              </div>
            </div>
                ...
                <div key={item.id}>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {item.hasDigitalForm && (
                        <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => navigate(`/transactions/${deal.id}/form/${item.id}`)}>
                          Edit Form
                        </Button>
                      )}
                      <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => { /* message office */ }}>
                        Message Office
                      </Button>
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
